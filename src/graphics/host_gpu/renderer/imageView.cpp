...
[[nodiscard]] vk::ImageUsageFlags DefaultColorViewUsage(vk::Format image_format) noexcept {
	// sRGB (and any color format without STORAGE_IMAGE) must not inherit
	// VK_IMAGE_USAGE_STORAGE_BIT from the image — VUID-02275.
	if (IsSrgbColorFormat(image_format) || !ImageViewOps::FormatSupportsStorage(image_format)) {
...
void CreateRenderTargetViews(RenderTextureVulkanImage& image) {
	const auto default_usage = DefaultColorViewUsage(image.format);
	CreateRenderTargetView(image, VulkanImage::VIEW_DEFAULT, vk::ComponentSwizzle::eIdentity,
	...
	if (image.samples == 1 && FormatSupportsStorage(image.format)) {
		...
		if (storage_format != vk::Format::eUndefined && FormatSupportsStorage(storage_format)) {
			CreateRenderTargetView(image, VulkanImage::VIEW_STORAGE,
			                       vk::ComponentSwizzle::eIdentity, vk::ComponentSwizzle::eIdentity,
			                       vk::ComponentSwizzle::eIdentity, vk::ComponentSwizzle::eIdentity,
...
vk::ImageView TextureCache::GetRenderTargetStorageView(RenderTextureVulkanImage& image,
	...
	if (exact && IsSrgbColorFormat(image.format)) {
		if (storage_compatible_format == vk::Format::eUndefined ||
		    !ImageViewOps::FormatSupportsStorage(storage_compatible_format)) {
