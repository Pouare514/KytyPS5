...
void BlitToSwapchain(CommandBuffer& buffer, VulkanImage& src_image,
	...
	if (encode_for_present) {
		...
		if (encode_cache.image == nullptr || encode_cache.device != graphics.device ||
			...
			encode_cache.image =
			    graphics.device.createImage(image_info, nullptr).value;
			const auto req = graphics.device.getImageMemoryRequirements(encode_cache.image);
			vk::MemoryAllocateInfo alloc {};
			alloc.sType           = vk::StructureType::eMemoryAllocateInfo;
			alloc.allocationSize  = req.size;
			alloc.memoryTypeIndex = 0;
			const auto mem_props  = graphics.physical_device.getMemoryProperties();
			...
			encode_cache.memory = graphics.device.allocateMemory(alloc, nullptr).value;
			(void)graphics.device.bindImageMemory(encode_cache.image, encode_cache.memory, 0);
			encode_cache.device = graphics.device;
